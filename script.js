// =====================================================================================
// ➡️ MODIFICATION START: Using Vercel Environment Variable for API URL
// =====================================================================================

// NOTE: We assume you set the Vercel ENV variable key as NEXT_PUBLIC_API_BASE_URL 
// and its value is the base URL (e.g., https://mini-iii-1-yx2c.onrender.com).

// Fallback to the hardcoded URL ONLY if environment variable is NOT set 
const HARDCODED_FALLBACK = 'https://mini-iii-5-q5cc.onrender.com/'; 
const VERCEL_INJECTED_URL = process.env.NEXT_PUBLIC_API_BASE_URL || process.env.VITE_API_BASE_URL;

// Base URL for your backend API
const API_BASE_URL = VERCEL_INJECTED_URL ? `${VERCEL_INJECTED_URL}/api` : `${HARDCODED_FALLBACK}/api`;
...
                </select>
                <button class="save-role-btn vault-btn primary" data-user-id="${user._id}" data-original-role="${user.role}" data-user-email="${user.email}">Save Role</button>
            </div>
...
            const userEmail = e.target.dataset.userEmail; // Get email from data-attribute
            const newRole = document.getElementById(`role-select-${userIdToUpdate}`).value; // Get selected value
            showGenericModal(
                "Confirm Role Change",
                `Are you sure you want to change the role of user **${userEmail}** to **"${newRole}"**?`,
                [
...
    if (unlockButton) {
        // console.log('Unlock button element found:', unlockButton); // Removed for production
        unlockButton.addEventListener('click', () => {
            // console.log('Unlock button clicked! Attempting to expand form.'); // Removed for production
            loginWindow.classList.add('expanded');
            loginFormWrapper.classList.remove('collapsed');
            // console.log('loginWindow classes after click:', loginWindow.classList.value); // Removed for production
            // console.log('loginFormWrapper classes after click:', loginFormWrapper.classList.value); // Removed for production
        });
    } else {
